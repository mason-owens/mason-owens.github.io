function onClicky() {
  ...
  var ez = document.querySelector("#ez_type").checked;
  var sub = document.querySelector("#num_sub").checked;

  if (mn_wrd_length > mx_wrd_length) {
    alert("Min is greater than max! Try again!");
    return;
  }
  else if (((mn_wrd_length * 4) - 1) >= mx_length) {
    alert("This is impossible and will not get a result.");
    return;
  }

  var listy = new Array(5);

  var approved_length_words = [];
  var counter = 0;
  for (var i=0; i < words.length; i++) {
    if ((words[i].length >= mn_wrd_length) && (words[i].length <= mx_wrd_length)) {
      approved_length_words.push(words[i]);
      counter++;
    }
  }
...
var lhand = ['1','2','3','4','5','q','w','e','r','t','a','s','d','f','g','z','x','c','v','b'];
var rhand = ['6','7','8','9','0','y','u','i','o','p','h','j','k','l','n','m'];
